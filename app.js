...
//stat timer if user clicks on start button
function startTimer(e){
    //Stop timer if user clicks again left button and active is set to false
    if(active === false){
        stopTimer(e.target);
        return true;
    }
...
function startIntervalMs(){
    ...
    interval = setInterval(()=>{
        //if one second is over -> milliseconds back to, call updateTime
        if(milliseconds === 10){
            milliseconds = 0;
            updateTime(seconds, secondsSpan);
        }
        //change text of milliseconds and increase milliseconds by one
...
//if reset button was clicked -> reset interval, change values back to starting point
function resetTimer(){
    clearInterval(interval);
    active = true;
    msSpan.innerHTML = "0";
